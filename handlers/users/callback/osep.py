...
async def osep_command(msg: CallbackQuery, state: FSMContext, session: sessionmaker):
    ...
    if used_osep:
        ...
        if not (await user_service.exist_osep_credentials(msg.from_user.id)):
            ...
            await msg.message.answer(
                msg_to_send, reply_markup=input_osep_data_keyboard()
            )
            return
        msg_to_send += (
            "ОСЭП не отслеживается\n"
...
async def osep_credentials_command(
...
async def osep_password_command(msg: Message, state: FSMContext, session: sessionmaker):
    ...
    logger.info(
        f"Пользователь {msg.from_user.id} {msg.from_user.username} поставил отслеживание ОСЭП!"
    )
...
async def watching_osep_command(
    msg: CallbackQuery, state: FSMContext, session: sessionmaker
):
    user_service = UserService(session)
...
async def watching_osep_command(
    ...
    await user_service.set_osep_status_used(msg.from_user.id, False)
    logger.info(
        f"Пользователь {msg.from_user.id} {msg.from_user.username} снял отслеживание ОСЭП!"
    )
