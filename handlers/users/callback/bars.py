...
async def bars_command(msg: CallbackQuery, state: FSMContext, session: sessionmaker):
    ...
    if used_bars:
        ...
        await msg.message.answer(
            msg_to_send, reply_markup=update_bars_data_keyboard(used_bars)
        )
        return
    else:
        if not (await user_service.exist_bars_credentials(msg.from_user.id)):
            ...
            await msg.message.answer(
                msg_to_send, reply_markup=input_bars_data_keyboard()
            )
            return
        msg_to_send += (
            "БАРС не отслеживается\n"
...
async def bars_credentials_command(
    msg: CallbackQuery, state: FSMContext, session: sessionmaker
):
    user_service = UserService(session)
...
async def bars_password_command(msg: Message, state: FSMContext, session: sessionmaker):
    ...
    try:
        ...
        logger.info(
            f"Пользователь {msg.from_user.id} {msg.from_user.username} ожидает 2FA код"
        )
    ...
    logger.info(
        f"Пользователь {msg.from_user.id} {msg.from_user.username} поставил отслеживание БАРСа!"
    )
...
async def process_2fa_code_command(
    msg: Message, state: FSMContext, session: sessionmaker
):
    user_service = UserService(session)
    ...
    if not res:
        ...
        # НЕ закрываем сессию — она может использоваться активным вотчером
        # Пользователь может попробовать снова или переподключиться через watching_bars
    else:
        logger.info(
            f"Пользователь {msg.from_user.id} {msg.from_user.username} поставил отслеживание БАРСа!"
        )
        cache = WatcherFactory.get_cache_service()
        bars_watcher = await WatcherFactory.create_bars_watcher(
            msg.from_user.id, auth, cache
        )
        await bars_watcher.start()
        await user_service.set_bars_status_used(msg.from_user.id, True)
        await msg.answer("Уведомления о БАРСе включены!")
...
async def watching_bars_command(
    msg: CallbackQuery, state: FSMContext, session: sessionmaker
):
    user_service = UserService(session)
...
async def watching_bars_command(
    ...
    await user_service.set_bars_status_used(msg.from_user.id, False)
    logger.info(
        f"Пользователь {msg.from_user.id} {msg.from_user.username} снял отслеживание БАРСа!"
    )
